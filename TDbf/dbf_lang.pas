...
function DbfStrLower(LocaleID: LCID; Str: PChar): PChar; // 04/29/2011 pb  CR 18895
function DbfStrUpper(LocaleID: LCID; Str: PChar): PChar; // 04/29/2011 pb  CR 18895
...
var
  kernel32_LCMapStringA: function(Locale: LCID; dwMapFlags: DWORD; lpSrcStr: PChar; cchSrc: Integer; lpDestStr: PChar; cchDest: Integer): Integer; stdcall; // 04/29/2011 pb  CR 18895

function DbfStrLower(LocaleID: LCID; Str: PChar): PChar; // 04/29/2011 pb  CR 18895
begin
  if Assigned(kernel32_LCMapStringA) then
    kernel32_LCMapStringA(LocaleID, LCMAP_LOWERCASE, Str, StrLen(Str), Str, StrLen(Str))
  else
    AnsiStrLower(Str);
  Result:= Str;
end;
...
begin
  if Assigned(kernel32_LCMapStringA) then
    kernel32_LCMapStringA(LocaleID, LCMAP_UPPERCASE, Str, StrLen(Str), Str, StrLen(Str))
  else
    AnsiStrUpper(Str);
  Result:= Str;
end;
...
initialization
  kernel32_LCMapStringA:= GetProcAddress(GetModuleHandle('kernel32.dll'), 'LCMapStringA'); // 04/29/2011 pb  CR 18895
